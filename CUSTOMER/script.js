...
async function loadCustomers() {
  try {
    const res = await fetch('customer_api.php');
    ...
    document.getElementById('tableWrap').innerHTML =
      `<div class="row" style="justify-content:center;color:#ef4444;padding:24px;">Failed to load customers: ${err.message}</div>`;
  }
}
...
function renderRows(list){
  ...
  if (list.length === 0) {
    wrap.innerHTML = `<div class="row" style="justify-content:center;color:#9ca3af;padding:24px;">No customers found.</div>`;
    return;
  }
  ...
  // Attach click handlers to the icon in each row
